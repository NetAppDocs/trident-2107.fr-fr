...
Oui, Astra Trident peut être installé hors ligne. Voir link:../trident-get-started/kubernetes-deploy.html["ici"^].
...
Oui. Astra Trident prend en charge le service Azure NetApp Files dans Azure, ainsi que le service Cloud Volumes Service dans AWS et GCP.
...
Créer un problème sur https://github.com/NetApp/trident["Trident Github"^] Et mentionner *RFE* dans le sujet et la description du problème.
...
Si vous avez des questions, des problèmes ou des demandes, n'hésitez pas à nous contacter en suivant notre http://netapp.io/slack["Mou"^] Équipe ou GitHub.
...
Un certain nombre de facteurs doivent être évalués si vous voulez rétrograder. Voir link:../trident-managing-k8s/downgrade-trident.html["la section sur la rétrogradation"^].
...
Vous pouvez définir le chiffrement sur le volume provisionné par Trident à l'aide du paramètre de chiffrement dans le fichier de définition back-end.
...
Depuis, nous proposons aujourd'hui la prise en charge de snapshots à la demande `ontap-nas`, `ontap-san`, `ontap-san-economy`, `solidfire-san`, `aws-cvs`, `gcp-cvs`, et `azure-netapp-files` pilotes backend.
